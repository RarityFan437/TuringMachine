"""Всё для парсинга кода."""
import re
from pathlib import Path
import src.functions as functions

def create_instruction_set(data):
    """Создает InstructionSet из данных инструкций."""
    return functions.InstructionSet(
        functions.Instruction(
            data[0][0],
            functions.Jump(*tuple(data[0][1:-1].split('!', 1))),
            functions.Shift(data[0][-1])
        ),
        functions.Instruction(
            data[1][0],
            functions.Jump(*tuple(data[1][1:-1].split('!', 1))),
            functions.Shift(data[1][-1])
        ),
        functions.Instruction(
            data[2][0],
            functions.Jump(*tuple(data[2][1:-1].split('!', 1))),
            functions.Shift(data[2][-1])
        )
    )
...
def Parse(file_name):
    ...
    try:
        ...
        raise RuntimeError("Permission denied. Maybe your machine's name has been lost from the cache.\nTry writing 'python main.py choose' and selecting your machine again.") from None
    ...
    for part in parts:
        ...
        if is_marked:
            ...
            found = functions_list.get(func_name)
            ...
            functions_list[func_name]['marked'] = [triplets[0], triplets[1], triplets[2]]
        else:
            # Записываем в словарь
            functions_list[func_name] = {}
            functions_list[func_name]['default'] = [triplets[0], triplets[1], triplets[2]]
    
    #Создаем функции
    for function_name, function_data in functions_list.items():
        default_set = create_instruction_set(function_data['default'])
        
        if 'marked' in function_data:
            marked_set = create_instruction_set(function_data['marked'])
            functions.Function(function_name, default_set, marked_set)
        else:
            functions.Function(function_name, default_set)
    
    # Закругляемся
    functions.Tape.current_function = functions.Function.functions[0]